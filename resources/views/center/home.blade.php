...
$name = $center->name;
$students = $center->students()->where('is_passed_out', false);
@endphp
@section('content')
...
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
            <h4 class="mb-sm-0">Study Center</h4>
                ...
                <table id="buttons-datatables" class="display table table-bordered" style="width:100%">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $student->applicant->lastname .' '. $applicant->othernames }}</td>
                            <td>{{ $student->applicant->student->matric_number }}</td>
                            <td>{{ $student->applicant->student->programme->name }}</td>
                            <td>{{ $student->applicant->student->email }} </td>
                            <td>{{ $student->applicant->student->academicLevel->level }} </td>
                            <td>{{ $student->applicant->phone_number }} </td>
                            <td>
                                <a href="{{ url('center/studentProfile/'.$student->slug) }}" class="btn btn-primary m-1"><i class= "ri-user-6-fill"></i> View Student</a>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
