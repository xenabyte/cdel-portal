...
class PaymentController extends Controller
{
    ...
    public function raveVerifyPayment(){
        ...
        try{
            ...
            if($paymentDetails['status'] == 'success'){
                if($this->processFlutterwavePayment($paymentDetails)){
                    alert()->success('Good Job', 'Payment successful')->persistent('Close');
                    if($student && !empty($studentId)){
                        $pdf = new Pdf();
                        $invoice = $pdf->generateTransactionInvoice($session, $studentId, $paymentId, 'single');
    ...
    public function monnifyVerifyPayment(Request $request){
        ...
        return $this->handleGenericPaymentVerification('monnify', $ref, '/', false);
    }
    ...
    private function handleGenericPaymentVerification($gateway, $reference, $redirectPath = '/', $returnAsJson = false){
        ...
        if ($gateway === 'monnify') {
            $monnify = new Monnify();
            $verifyInvoice = $monnify->verifyInvoice($reference);
    ...
    private function genericVerificationResponse($message, $asJson, $success, $redirectPath, $paymentType = null, $student = null, $programmeCategories = [], $payment = null){
        if ($asJson) {
            return [
    ...
    // public function monnifyVerifyPayment(Request $request){
    //     Log::info("**********************Monnify Verifying Payment**********************");
