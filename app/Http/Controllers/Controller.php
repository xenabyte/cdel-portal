...
class Controller extends BaseController
{
    ...
    public function checkProbation($student, $cgpa, $currentGPA, $previousGPA)
    {
        ...
        if ($cgpa < 1.50) {
            ...
            // // Notify the Registrar and Student
            // \Mail::to($student->email)->send(new ProbationNotification($student));
        }

        if ($student->level >= 200 && $cgpa < 1.50) {

            if ($currentGPA < 1.5 && $previousGPA < 1.5) {
                $student->academic_status = 'Withdrawn';
                $student->save();

                // // Notify the Registrar and Student
                // \Mail::to($student->email)->send(new WithdrawalNotification($student));
            }
        }
    } 
